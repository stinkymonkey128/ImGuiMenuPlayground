...
void GUIH::drawRect(int x, int y, int w, int h, int r, ImU32 color, ImDrawCornerFlags flags) {
    Object o;
    o.x = x + pos.x - 8;
...
void GUIH::drawMessage(ImFont* font, float fontSize, const char* text, int x, int y, ImU32 color) {
    Object o;
    o.fontSize = fontSize;
    o.x = x + pos.x - 8;
...
void GUIH::drawGradient(int x, int y, int w, int h, ImU32 bottomL, ImU32 bottomR, ImU32 topL, ImU32 topR) {
    Object o;
    o.x = x + pos.x - 8;
    o.y = y + pos.y - (NOTITLEBAR ? 8 : 29);
    o.w = w;
...
void GUIH::drawImage(LPDIRECT3DTEXTURE9 image, int x, int y, int w, int h, ImU32 col) {
    Object o;
    o.image = image;
    o.x = pos.x + x - 8;
    o.y = pos.y + y - 8;
...
bool GUIH::inBound(HWND hwnd, ImVec2 topLeft, ImVec2 botRight) {
...
bool GUIH::inBound(HWND hwnd, int x, int y, int w, int h) {
    return inBound(hwnd, ImVec2(x, y), ImVec2(x + w, y + h));
}

ImVec2* GUIH::drawHText(int minX, int maxX, int y, const char* texts[], int num, ImFont* font, int fontSize, int selec, ImU32 off, ImU32 on) {
    ImVec2 textSize[15];
...
int GUIH::drawVNavBar(HWND& hwnd, int x, int minY, int maxY, LPDIRECT3DTEXTURE9 icons[], int num, ImVec2 iconSize, ImVec2 highlightSize, int& selec) {
...
int GUIH::drawHSubBar(HWND& hwnd, int minX, int maxX, int y, const char* texts[], int num, int& selec, ImFont* font, int fSize) {
    ImVec2 pos(minX, y);
    ImVec2 textWid[32];
    int totalWidth = 0;
...
int GUIH::drawHBarFSep(HWND& hwnd, int midX, int y, int forceSep, const char* texts[], int num, int& selec, ImFont* font, int fSize) {
    ImVec2 textWid[32];
    int totalWidth = 0;
...
bool GUIH::drawCheckbox(HWND& hwnd, ImVec2 size, ImVec2 pos, bool& toggle, int borderpx, int roundness, ImU32 background, ImU32 border, ImU32 off, ImU32 on, ImU32 highlight) {
    drawRect(pos.x, pos.y, size.x, size.y, roundness, border);
    drawRect(pos.x + borderpx, pos.y + borderpx, size.x - borderpx * 2, size.y - borderpx * 2, roundness, background);
    ...
    drawRect(pos.x + borderpx * 2, pos.y + borderpx * 2, size.x - borderpx * 4, size.y - borderpx * 4, roundness, col);
